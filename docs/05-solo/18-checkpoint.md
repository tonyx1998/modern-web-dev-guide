...
description: Mandatory checkpoint quiz for Chapter 10 — Solo / Personal. 5 random questions drawn from a 15-question bank. Pass to unlock Chapter 12.
---
...
You must pass (≥ 67%) to unlock the Next button and Chapter 11 in the sidebar.
...
<Question
  prompt="A friend describes their idea: 'a website where running friends log in, draw shared routes on a map, and comment on each other's runs.' Which project type from Chapter 11 does that actually map to?"
  options={[
...
→ Continue to [Chapter 12: Startup / Small Co.](/docs/startup) to see how the workflow scales when you're not alone anymore.
