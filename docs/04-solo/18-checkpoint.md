...
description: Mandatory checkpoint quiz for Chapter 4 — Solo / Personal. 5 random questions drawn from a 15-question bank. Pass to unlock Chapter 5.
---
...
In **Beginner mode**, you must pass (≥ 60%) to unlock the Next button and Chapter 5 in the sidebar. Switch the reading-level pill (top right) if you'd rather skip.
...
<Question
  prompt="A friend describes their idea: 'a website where running friends log in, draw shared routes on a map, and comment on each other's runs.' Which project type from Chapter 4 does that actually map to?"
  options={[
...
→ Continue to [Chapter 5: Startup / Small Co.](/docs/startup) to see how the workflow scales when you're not alone anymore.
